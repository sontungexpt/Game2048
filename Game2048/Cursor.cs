...
namespace Game2048
{
    ...
    internal static class Cursor
    {
        ...
        public static int InputAt(Coord positionInput, int maxNumberOfCharacter, int minSize, int maxSize)
        {
            try
            {
                ...
                while (c != ConsoleKey.Enter || currentNumber < minSize)
                {
                    ...
                    if (length >= maxNumberOfCharacter)
                        continue;
                    if (c >= (ConsoleKey)'0' && c <= (ConsoleKey)'9' && nextNumber <= maxSize && length < maxNumberOfCharacter)
                    {
                        length++;
        ...
        public static void WriteAt(dynamic s, Coord writingPosition, ConsoleColor charColor = ConsoleColor.White)
        {
            try
            {
        ...
        public static void WriteContinueAt(dynamic s, Coord writingPosition, ConsoleColor charColor = ConsoleColor.White)
        {

            try
            {
                Console.ForegroundColor = charColor;
                Console.SetCursorPosition(Cursor.Current().x + writingPosition.x, Cursor.Current().y + writingPosition.y);
                Console.Write(s);
                Console.ResetColor();
            }
            catch (ArgumentOutOfRangeException e)
        ...
        //delete the existed character from current coordinates to end of Length Of Console Screen
        ...
        //delete the existed character from current coordinates with the numbers of deleting characters
